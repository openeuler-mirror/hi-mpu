...
static void OsSicrInit(void)
{
    ...
    sicrWaker.value = GIC_REG_READ(regAddr);
    while (sicrWaker.bits.sleep == 1) {
        sicrWaker.value = GIC_REG_READ(regAddr);
    }
}

static void OsSicrSetIntGroup(U32 coreId, U64 intId, enum SicGroupType groupId)
{
...
static U32 OsSiccEnableSre(void)
{
...
static void OsSiccCfgIntPreempt(void)
{
    U64 tmp = 0;
...
static void OsSiccEnableGroup1(void)
{
    U64 tmp = 0;
...
static void OsSiccCfgPriorityMask(void)
{
    U64 tmp = 0;
...
static void OsSicdSetIntGroup(U32 intId, enum SicGroupType groupId)
{
    U64 group0RegAddr;
    U64 modRegAddr;
    U32 group0RegTmp;
...
static void OsSicSetGroup(U32 intId, enum SicGroupType groupId)
{
...
static U32 OsSicInitLocal(void)
{
    U32 ret;
    U32 intId;
...
static void OsSicInitGlobal(void)
{
    U32 intId;
...
#elif (OS_GIC_VER == 2)
static int IsrRegister(U32 intNo, U32 pri)
{
    U32 bitn, reg, shift;
    U32 *addr;
...
static void OsGicInitCpuInterface(void)
{
    int i;
    U32 val;
    ...
    for (i = 0; i < 32; i += 4) { // 32 bit 4 offset
...
U32 OsHwiInit(void)
{
    ...
    if (SRE_GetCoreID() == LIBCK_CORE_ID) {
        OsSicInitGlobal();
    }
#elif (OS_GIC_VER == 2)
