...
void mcs_peripherals_shutdown(void)
{
    ...
    SRE_HwiDisable(SAMPLE_UART4_INT);
#if defined(SAMPLE_UART_DMA_TEST) && SAMPLE_UART_DMA_TEST == 1
...
static int sample_uart_interrupt_receive(unsigned int core)
{
    ...
    /* route */
    if (core == SMPLE_UART_IRQ_CORE) {
        SRE_HwiSetAttr(SAMPLE_UART4_INT, SAMPLE_UART4_PRIO, OS_HWI_MODE_ENGROSS);
        SRE_HwiCreate(SAMPLE_UART4_INT, (HwiProcFunc)(uintptr_t)sample_uart4_recv_isr, 0);
        OsHwiAffinitySet(SAMPLE_UART4_INT, 1 << core);
        SRE_HwiEnable(SAMPLE_UART4_INT);
    }
#endif
    return BM_OK;
}
