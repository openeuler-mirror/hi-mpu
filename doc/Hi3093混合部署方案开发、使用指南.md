...
**步骤1**：修改`hi-mpu/open_source/open-amp/lib/include/openamp/rpmsg_virtio.h`中：范围最大为1K，修改地方如图2-1所示。
...
2. 针对于UniProton中buffer大小可配置，修改地方`:hi-mpu/src/real_time/UniProton/UniProton-v1.0.1/demos/hi3093/component/open-amp/lib/include/openamp/rpmsg_virtio.h`中RPMSG_BUFFER_SIZE；修改完成后，执行如下命令:
    ```	
...
**步骤2**: 打开现有*.dts文件，增加或者修改驱动外设中属性（驱动外设支持参考特别说明中），属性支持如下说明；支持属性：domain、channel。

1. domain参数值说明：0x0：非实时侧使用；0x1：代表实时侧使用。
2. channel：参数值：<0x0 0x7> : 代表通道0到通道7赋予实时侧使用。
3. 仅支持数值小于10的数字。
...
现有支持uart、gmac、timer、gpio、i2c、spi、watchdog、dmac、localbus，只有dmac支持channel。其他驱动仅支持domain。

## 2.5 混合部署下电指南

在混合部署方案中，执行混合部署升级指南操作后，需要进行等待10s，然后在执行rpmsg_main的串口中，执行Ctrl+C操作，就可以正常下从核CPU的电。
...
主核调用流程如下步骤所示：
    ...
    **步骤4** 在回调函数endpoint_cb中调用
        get_zero_cpy_data_addropenamp_send_message。
