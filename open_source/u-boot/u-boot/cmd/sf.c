...
#include "../drivers/mtd/spi/hi309x_spi.h"
#endif
			...
			char *const argv[])
{
	...
	int ret;
#ifdef CONFIG_HI3093_SPI
    u8 need_resume_bank0 = 1; /* 默认需要恢复，probe不需要恢复*/
#endif
...
#ifdef CONFIG_HI3093_SPI
    ret = spi_flash_cmd_prep(flash);
    if (ret) {
        puts("spi_flash_cmd_prep fail'\n");
        ret = -1;
        goto done;
    }
#endif
...
#ifdef CONFIG_HI3093_SPI
    if (need_resume_bank0) {
        exec_cmd_resume_bank0(flash);
    }
    ret = spi_flash_cmd_release(flash);
    if (ret) {
        puts("spi_flash_cmd_release fail'\n");
        ret = -1;
        goto done;
    }
#endif
