...
/* 1711 add for spansion or cypress (s70fs01gs) */
struct spec_flash_para {
    u8 use_spec_rdsr;
    u8 use_4b_flag; /* 使用四字节地址模式 */
    u8 use_spec_erase;
    u8 use_spec_read;
    u8 cmd_wrar;
    u8 cmd_rdar;
    u8 cmd_4b_need_wren; /* 四字节地址切换之前是否需要写使能 */
    u8 cmd_enter4b;
    u8 cmd_exit4b;
    u8 rdar_dummy_byte;
    u8 flash_type;       /* scan时存储特殊处理的flash 类型 */
    uint bound_mem_addr; /* 如s70fs01gs的64M使用不同的寄存器 */
    uint opr_flash_mem_addr;
};
#endif
...
#ifdef CONFIG_HI3093_SPI
    /* 3093 add for spansion or cypress (s70fs01gs) */
    struct spec_flash_para spec_para;
#endif
};
