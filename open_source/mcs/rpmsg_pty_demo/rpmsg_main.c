...
{
    app_exiting = true; // The cleanup operation is performed after the main loop exits.
}
...
static int check_input_params(char *cpu_id, char *target_binfile, char *target_binaddr)
{
    ...
    if (!(strtol(cpu_id, NULL, STR_TO_DEC) == 3)) {
...
int main(int argc, char **argv)
{
    ...
    printf("cpu:%d, entry:%lx, path:%s\n", client_os.cpu_id, client_os.entry, client_os.path);
