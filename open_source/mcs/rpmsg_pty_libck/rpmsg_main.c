...
#define RPMSG_ENDPOINT_PTY "virt_serial"
...
/** Processing data sent from the other processor */
...
static int rpmsg_app_master(struct client_os_inst *client)
{
    ...
    int register_endpoint_pty = openamp_create_pty_endpoint(RPMSG_ENDPOINT_PTY, pty_endpoint_cb);
    if (register_endpoint_pty < 0) {
        return -1;
    }
    ...
    tick_count = 15000;
    while (!openamp_endpoint_ready(RPMSG_ENDPOINT_PTY)) {
        if (tick_count < 0) {
            printf("The %s endpoint isn't ready\n", RPMSG_ENDPOINT_PTY);
            return -1;
        }
