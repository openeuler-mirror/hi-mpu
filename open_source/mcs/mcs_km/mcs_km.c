...
#define IOC_CLOSE_OPENAMP	_IOW(MAGIC_NUMBER, 3, int)
#define IOC_MAXNR		4
#define IPI_MCS			8
...
typedef struct phy_address_node {
    unsigned long phy_start;
    unsigned long phy_end;
    struct phy_address_node *next;
} phy_address_node, *phy_addr_node;

typedef struct phy_addr_list {
    phy_addr_node first;
    phy_addr_node last;
    unsigned int size;
} phy_addr_list;

struct phy_addr_list phy_list;

static void init_phy_list(void)
{
    phy_list.first = phy_list.last = (phy_address_node *)vmalloc(sizeof(phy_address_node));
    if (phy_list.first == NULL) {
        return;
    }
    phy_list.first->next = NULL;
    phy_list.size = 0;
	return;
}

static int find_value(unsigned long phy_start, unsigned long phy_end)
{
    if (phy_list.first == NULL) {
        return 0;
    }

    phy_address_node *node = phy_list.first->next;
    while (node != NULL) {
        if (phy_start > node->phy_end) {
            node = node->next;
            continue;
        }

        if (phy_end < node->phy_start) {
            node = node->next;
            continue;
        }
        return 1;
...
static void insert_push_back(unsigned long phy_start, unsigned long phy_end)
{
    phy_address_node *node = (phy_address_node *)vmalloc(sizeof(phy_address_node));
    if (node == NULL) {
        return;
    }

    node->phy_start = phy_start;
    node->phy_end = phy_end;
    node->next = NULL;

    phy_list.last->next = node;
    phy_list.last = node;

    unsigned int size_new = phy_list.size + 1;
    phy_list.size = size_new;
...
static int mcs_release(void)
{
    phy_address_node *node = phy_list.first->next;
    while (node != NULL) {
        phy_list.first->next = node->next;
        vfree(node);
        node = phy_list.first->next;
    }

    phy_list.last = phy_list.first;
    phy_list.size = 0;
	atomic_set(&init_state, 0);
	return 0;
}
...
{
	int status = atomic_read(&init_state);
	if (status == 0) {
		atomic_set(&init_state, 1);
		init_phy_list();
	}
	...
	phys_addr_t phy_start = offset;
	phys_addr_t phy_end  = offset + (phys_addr_t)size - 1;
	if (phy_start > phy_end) {
		return -EINVAL;
	}

	status = find_value(phy_start, phy_end);
	if (status == 1) {
		return -EINVAL;
	}
	insert_push_back(phy_start, phy_end);
