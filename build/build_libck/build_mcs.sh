...
CROSS_COMPILE_GCC_ARM_LIBCK=aarch64-openeuler-linux-gcc
...
build_libmetal()
{
    ...
    cmake ${LIBMETAL_PATH} -DCMAKE_C_COMPILER=${CROSS_COMPILE_GCC_ARM_LIBCK} -DCMAKE_TOOLCHAIN_FILE=$LIBMETAL_PATH/cmake/platforms/libck_libmetal_arm64_gcc.cmake -DWITH_DOC=OFF -DWITH_EXAMPLES=OFF -DWITH_TESTS=OFF -DWITH_DEFAULT_LOGGER=OFF -DWITH_SHARED_LIB=OFF
...
build_openamp()
{
    ...
    cmake ${OPENAMP_PATH} -DCMAKE_C_COMPILER=${CROSS_COMPILE_GCC_ARM_LIBCK} -DCMAKE_TOOLCHAIN_FILE=${OPENAMP_PATH}/cmake/platforms/libck_openamp_arm64_gcc.cmake -DDEMO_TARGET=libck
...
build_mcs()
{
    ...
    cmake -S . -B build -DCMAKE_C_FLAGS="-fstack-protector-strong -s" -DCMAKE_C_COMPILER=aarch64-openeuler-linux-gcc -DDEMO_TARGET=rpmsg_pty_libck
    cd ${MCS_PATH}/build
    make VERBOSE=1
