 ...
 * and this function is defined only when LOSCFG_QUEUE_DYNAMIC_ALLOCATION is defined in the LiteOS system.
 * The queues available in freertos are determined by its system memory.
 ...
 * Do not read or write a queue in unblocking modes such as interrupt except freertos.
 ...
 * Do not read or write a queue in unblocking modes such as an interrupt except freertos.
 * This API cannot be called before the LiteOS is initialized.
 * The argument timeout is a relative time.
 * Do not call this API in software timer callback.
 * The buffer_size parameter is not used in the freertos system and
